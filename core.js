...
const completed=courses.filter(c=>state.completed.includes(c.id));
const completedHours=completed.reduce((sum,c)=>sum+(c.hours??0),0);
const unknownCompletedHours=completed.some(c=>c.hours===null);
return courses.filter(c=>state.selected[c.id]).map(c=>({course:c,unknown:c.prerequisites===null,
missing:(c.prerequisites||[]).filter(id=>!state.completed.includes(id)),
missingCorequisites:(c.corequisites||[]).filter(id=>!state.completed.includes(id)&&!state.selected[id]),
minimumHours:c.minCompletedHours||0,completedHours,
hoursShortfall:Math.max(0,(c.minCompletedHours||0)-completedHours),unknownCompletedHours}));
}
