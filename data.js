...
(function (root) {
  ...
  const courses = [];
  // s(dia, ...índices de slots) → encontros; o(turma, ...encontros) → turma.
  const s = (day, ...rows) => rows.map((row) => ({ day, start: slots[row][0], end: slots[row][1] }));
  const o = (id, ...parts) => ({ id, meetings: parts.flat() });
  const biweekly = (meetings, extra = {}) => meetings.map((m) => ({ ...m, frequency: 2, ...extra }));
  function c(period, id, short, name, sections = [], note = '', offered = true) {
    courses.push({ period, id, short, name, sections, note, offered, hours: null, prerequisites: null });
  }

  const NO_FIXED = 'Sem horário fixo.';
  ...
  c(1, 'FEELT32102', 'EXT1', 'Extensão 1');
  ...
  c(2, 'FEELT32202', 'EXT2', 'Extensão 2');
  ...
  c(3, 'FEELT31302', 'E-CE1', 'Experimental de Circuitos Elétricos 1',
    ['1.1', '1.2', '2.1', '2.2', '3.1', '3.2', '4.1', '4.2'].map((id) => {
      const n = Number(id[0]);
      return o(id, biweekly(s(n <= 2 ? 0 : 1, ...(n % 2 ? [6, 7] : [8, 9]))));
    }),
    'Aulas quinzenais. Calendário de alternância não informado; sobreposições são potenciais.');
  ...
  c(3, 'FEMEC39102', 'MCS', 'Mecânica dos Sólidos', [o('1', s(3, 9, 10))], 'Período confirmado no fluxograma: 3º. O cabeçalho da página 5 do PDF de horários é inconsistente.');
  c(3, 'FAMAT31033', 'ESTAT', 'Estatística', [o('1', s(0, 0, 1), s(1, 0, 1))]);
  c(3, 'FEELT32302', 'EXT3', 'Extensão 3');
  ...
  c(4, 'IBTEC39301', 'BIOQ', 'Bioquímica',
    ['B1', 'B2'].map((id) => o(id, s(4, 9, 10, 11), biweekly(s(4, 2, 3), { firstDate: id === 'B1' ? '2026-09-04' : '2026-09-11' }))),
    'Teoria semanal. Laboratório quinzenal no campus Umuarama: B1 a partir de 04/09; B2 a partir de 11/09.');
  c(4, 'FEQUI39401', 'FT', 'Fenômenos de Transporte', [o('1', s(0, 10, 11), s(1, 10, 11)), o('2', s(0, 4, 5), s(1, 4, 5))]);
  c(4, 'FEELT32402', 'EXT4', 'Extensão 4');
  ...
  // O PDF de horários agrupa 9º e 10º; o fluxograma separa TCC e EXT5 (9º) de estágio (10º).
  c(9, 'FEELT31908', 'TCC', 'Trabalho de Conclusão de Curso', [], `${NO_FIXED} Exige pelo menos 2.700 horas curriculares concluídas.`);
  c(10, 'FEELT31005', 'ESTÁGIO', 'Estágio Obrigatório em Engenharia Biomédica', [], `${NO_FIXED} Exige pelo menos 2.300 horas curriculares concluídas.`);
  c(9, 'FEELT32903', 'EXT5', 'Atividades Curriculares de Extensão V', [], `${NO_FIXED} Alocada no 9º período pelo fluxograma.`);
  ...
  const byShort = new Map(courses.map((course) => [course.short, course]));
  const byNumber = new Map();
  for (const [number, short, hours] of curriculum) {
    const course = byShort.get(short);
    if (!course) throw Error('Disciplina curricular não encontrada: ' + short);
    Object.assign(course, { curriculumNumber: number, hours, prerequisites: [], corequisites: [], minCompletedHours: 0 });
    byNumber.set(number, course);
  }

  // Setas simples: pré-requisitos. Setas duplas: co-requisitos.
  for (const [number, required] of [[8, 1], [15, 8], [23, 15], [33, 24], [56, 49], [59, 52], [60, 53]]) {
    byNumber.get(number).prerequisites.push(byNumber.get(required).id);
  }
  for (const [a, b] of [[10, 11], [17, 18], [19, 20], [24, 25], [29, 30], [33, 34], [39, 40], [43, 44]]) {
    byNumber.get(a).corequisites.push(byNumber.get(b).id);
    byNumber.get(b).corequisites.push(byNumber.get(a).id);
  }
  ...
  for (const course of courses.filter((item) => /^EXT[1-5]$/.test(item.short))) {
  ...
  // Intervalos exibidos na grade antes do slot indicado.
  const breaks = [
    { before: '13:10', label: '12:20–13:10', name: 'Almoço' },
    { before: '19:00', label: '18:30–19:00', name: 'Jantar' },
  ];

  root.BIO_DATA = { courses, slots, breaks, semester: '2026.2' };
